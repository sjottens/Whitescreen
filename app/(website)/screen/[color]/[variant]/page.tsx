// app/(website)/screen/[color]/[variant]/page.tsx
// English-only screen variant pages: /screen/[color]/[variant]
// Example: /screen/red/4k, /screen/blue/oled, etc.
// Generates 9 colors × 3 variants = 27 English pages
...
export async function generateStaticParams() {
  const colors = COLOR_TOOLS.map(c => c.id);
  const variants = Object.keys(SCREEN_VARIANTS);

  const params: PageParams[] = [];
  for (const color of colors) {
    for (const variant of variants) {
      params.push({ color, variant });
    }
  }
  return params;
}
...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  const { color, variant } = await props.params;
...
export default async function ScreenVariantPage(props: PageProps) {
  ...
  const breadcrumbs = [
    ...
    { name: variantData.titleSuffix, path: `/screen/${color}/${variant}` },
  ];
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(breadcrumbSchemaMultilingual(breadcrumbs, 'en')),
        }}
      />
